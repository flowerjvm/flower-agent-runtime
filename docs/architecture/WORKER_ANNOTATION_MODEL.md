# Worker Annotation Model

This document records a future usability direction for `flower-action-runtime`.
...
`flower-action-runtime` should eventually offer the same kind of convenience for
...
This gives a host application or worker profile a named action catalog.

```java
@ActionCatalog("archdox")
class ArchDoxActionCatalog {
}
```
...
```text
@ActionCatalog / @ActionWorker / @Action
-> scanner
-> ActionDefinition
-> ActionInvoker
-> ActionRegistry
...
```text
@Action("document.review")
-> action runtime checks action policy and audit requirements
...
`flower-action-runtime` owns whether a proposed business action may execute.
...
Controlled execution backend
  ...
  = flower-action-runtime-workflow for observable control stages.
  = future flower-action-runtime-eventloop for durable waits, approval resume,
    and callback-driven actions.
```
...
3. Build flower-action-runtime-workflow for observable action execution.
4. Validate with real host actions.
5. Add event-loop backend only when durable waits are proven necessary.
6. Add testkit/check support.
7. Add Spring annotation convenience.
8. Add REST/MCP/catalog adapters later.
```
